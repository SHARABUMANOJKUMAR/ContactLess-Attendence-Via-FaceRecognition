...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Validate that authenticated user matches the email in request
    if (user.email !== email) {
    ...
    // Fetch ALL user emails from the database to send notifications to everyone
    ...
    if (profilesError) {
      ...
      return new Response(JSON.stringify({ success: false, error: "Failed to fetch user list" }), {
        status: 500,
        headers: { "Content-Type": "application/json", ...corsHeaders },
      });
    }

    console.log(`Sending attendance notifications to ${allProfiles.length} users`);

    const statusColor = status === "present" ? "#10b981" : "#ef4444";
    const statusEmoji = status === "present" ? "✅" : "❌";

    // Send email to ALL users in the database
    const emailPromises = allProfiles.map(async (profile) => {
      const isCurrentUser = profile.email === email;
      ...
      return resend.emails.send({
        ...
        subject: isCurrentUser 
          ? `Attendance ${status === "present" ? "Marked" : "Alert"} - ${rollNumber}`
          : `${name} Marked Attendance - ${rollNumber}`,
        html: `
          <!DOCTYPE html>
          <html>
          <head>
          ...
          <body>
            <div class="container">
              ...
              <div class="content">
                <h2 style="color: #1f2937; margin-top: 0;">${isCurrentUser ? 'Attendance Confirmation' : 'Attendance Notification'}</h2>
                <p style="color: #4b5563; font-size: 16px;">Hello ${recipientName},</p>
                <p style="color: #4b5563; font-size: 16px;">
                  ${isCurrentUser 
                    ? `Your attendance has been ${status === "present" ? "successfully recorded" : "marked as absent"}.`
                    : `<strong>${name}</strong> (${rollNumber}) has ${status === "present" ? "marked their attendance as present" : "been marked as absent"}.`
                  }
                </p>
                ...
                <p style="color: #6b7280; margin-top: 30px; font-size: 14px;">
                  ${isCurrentUser 
                    ...
                    : "This is an automated notification. All users receive real-time attendance updates."
                  }
                </p>
              </div>
              <div class="footer">
    ...
    return new Response(JSON.stringify({ 
      ...
      details: { successCount, failedCount, totalUsers: allProfiles.length }
    }), {
      status: 200,
      headers: {
